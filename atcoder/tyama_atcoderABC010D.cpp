...
int main(){
	...
	scanf("%d%d%d",&V,&G,&E);
	Graph g(V+1);
	for(;G--;)scanf("%d",&s),g[s].push_back(Edge(s,V,1)),g[V].push_back(Edge(V,s,0));
	for(;E--;)scanf("%d%d",&s,&t),g[s].push_back(Edge(s,t,1)),g[t].push_back(Edge(t,s,1)),g[s].push_back(Edge(s,t,0)),g[t].push_back(Edge(t,s,0));
	printf("%d\n",maximumFlow(g,0,V));
}