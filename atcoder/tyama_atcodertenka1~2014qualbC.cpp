...
int main(){
	...
	for(int i=0;i<1<<n;i++){
		...
		for(int l=1;l<n;l++){
			...
			vector<int> black(n);
			for(int k=0;k<n;k++){
				for(int d=0;d<MD;d++)if(0<=l-1+D[d][0]&&l-1+D[d][0]<n && 0<=k+D[d][1]&&k+D[d][1]<n && before[l-1+D[d][0]][k+D[d][1]]=='#')black[k]++;
			}
		...
		vector<int> black(n);
		for(int k=0;k<n;k++){
			for(int d=0;d<MD;d++)if(0<=n-1+D[d][0]&&n-1+D[d][0]<n && 0<=k+D[d][1]&&k+D[d][1]<n && before[n-1+D[d][0]][k+D[d][1]]=='#')black[k]++;
		}
