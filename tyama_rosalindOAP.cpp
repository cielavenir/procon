...
void alignment(string &x, string &y){
	...
	string tx,ty;
	int i,j,k;
	for(i=1;i<a.size();i++){a[i][0]=a[i-1][0]-0;back[i][0]=make_tuple(i-1,0,0);}
	for(j=1;j<a[0].size();j++){a[0][j]=a[0][j-1]-2;back[0][j]=make_tuple(0,j-1,1);}
	for(i=1;i<a.size();i++){
		for(j=1;j<a[0].size();j++){
			vi Z{x[i-1]==y[j-1]?(a[i-1][j-1]+1):(a[i-1][j-1]-2),a[i-1][j]-2,a[i][j-1]-2};
	...
	vi alast=a[x.length()];
	vi::iterator z=max_element(alast.begin(),alast.end());
	int n=x.length(),m=z-alast.begin();
	vi t;
	for(;n||m;){
