...
def alignment(x, y)
	...
	back = Array.new(x.length+1){Array.new(y.length+1)};back[0][0]=[0,0,'']
	tx = ""; ty = ""; t=""

	#DP
	1.step(a.length-1){|i| a[i][0] = a[i-1][0] - (back[i-1][0][2]=="a"?0:5);back[i][0]=[i-1,0,  "a"]}
	1.step(a[0].length-1){|j| a[0][j] = a[0][j-1] - (back[0][j-1][2]=="b"?0:5);back[0][j]=[0,  j-1,"b"]}
	1.step(a.length-1){|i|
