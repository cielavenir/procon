...
int main(){
	...
	for(;cin>>N;){
		string s,t;
		cin>>t>>s>>t;
		vector<vector<int> >_m(s.size());
		{
			vector<string>V(N);
			for(int i=0;i<N;i++)cin>>V[i];
			for(int j=0;j<s.size();j++){
				_m[j].resize(N);
				for(int i=0;i<N;i++)_m[j][i]=V[i].find_first_of(s[j])==string::npos?99999:0;
			}
		}
		int ma=max((int)s.size(),N);
		...
		for(int i=0;i<s.size();i++){
			for(int j=0;j<N;j++){
#if MODE==1
				if(as[i][j]==1)ret+=v2[i][j];
#elif MODE==2
				if(mat(i,j)==0)ret+=v[i][j];
#endif
			}
		}
		puts(ret?"False":"True");
